...
CREATE TABLE musics (
	id SERIAL PRIMARY KEY,
	media_id BIGINT unsigned NOT NULL,
	name VARCHAR(100) NOT NULL, -- тут будем хранить название трека

	INDEX (name), -- индекс по полю name для поиска
    FOREIGN KEY (media_id) REFERENCES media(id)
);
...
CREATE TABLE videos (
	id SERIAL PRIMARY KEY,
	media_id BIGINT unsigned NOT NULL,
	name VARCHAR(100) NOT NULL, -- тут будем хранить название
	description TEXT NOT NULL, -- тут будем хранить описание
	count_views INT DEFAULT NULL, -- количество просмотров
...
CREATE TABLE games (
	id SERIAL PRIMARY KEY,
	media_id BIGINT unsigned NOT NULL,
	name VARCHAR(100) NOT NULL, -- тут будем хранить название игры
	description TEXT NOT NULL, -- тут будем хранить описание игры
	rating ENUM('1', '2', '3', '4', '5'), -- рейтинг игры

	INDEX (name), -- индекс по полю name для поиска
	INDEX (rating), -- индекс по полю rating для поиска
    FOREIGN KEY (media_id) REFERENCES media(id)
);
